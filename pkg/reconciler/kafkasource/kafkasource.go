...
import (
	...
	"github.com/knative/eventing-sources/pkg/reconciler/kafkasource/resources"
...
// Add creates a new KafkaSource Controller and adds it to the Manager with
...
func (r *reconciler) Reconcile(ctx context.Context, object runtime.Object) error {
	...
	//     - Will be garbage collected by K8s when this KafkaSource is deleted.

	src.Status.InitializeConditions()

	/*_, err := cron.ParseStandard(src.Spec.Brokers)
	if err != nil {
...
func (r *reconciler) getReceiveAdapter(ctx context.Context, src *v1alpha1.KafkaSource) (*v1.Deployment, error) {
	...
	if err != nil {
		logging.FromContext(ctx).Desugar().Error("Unable to list kafkas: %v", zap.Error(err))
		return nil, err
	}
