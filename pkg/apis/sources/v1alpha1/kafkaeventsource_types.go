...
const (
	...
	// KafkaConditionValidSchedule has status True when the KafkaSource has been configured with a valid schedule.
	KafkaConditionValidSchedule duckv1alpha1.ConditionType = "ValidSchedule"
...
func (s *KafkaSourceStatus) MarkSchedule() {
	kafkaSourceCondSet.Manage(s).MarkTrue(KafkaConditionValidSchedule)
}

// MarkInvalidSchedule sets the condition that the source does not have a valid schedule configured.
func (s *KafkaSourceStatus) MarkInvalidSchedule(reason, messageFormat string, messageA ...interface{}) {
	kafkaSourceCondSet.Manage(s).MarkFalse(KafkaConditionValidSchedule, reason, messageFormat, messageA...)
}*/
